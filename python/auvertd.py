...
def getCollFiles(collPath): # Get a list of all of the files contained in a collection
    ...
    print collPath
    for branch in os.walk(collPath):
        for filePath in branch[2]:
            
            print branch[0]+filePath
        #print os.path.join(path[0],path[2])
    exit()
...
def getColls(collPath):

    AUVERT_PERL_BIN='/home/aupro/ap/perl/bin/fileAuvertCollection.pl'

    AMFAUVOUTPATH = amfAuvOutPath

#lambda x: x % 3 == 0, foo
    EXCLUDEDCOLLS=[]
    EXCLUDEFILE='/home/aupro/var/auvert/excluded.txt'
# Open and read from the file
    excludeFH = open(EXCLUDEFILE)
    for excludedPath in excludeFH.readlines():
        # Exclude comments
        if re.search('^#',excludedPath):
            continue
        EXCLUDEDCOLLS.append(re.sub(AMFCOLLROOTPATH,'',re.sub(r'\n$','',re.sub('~',os.environ['HOME'],excludedPath))))
        excludeFH.close()

    print EXCLUDEDCOLLS

    COLLS=[]
    for path in os.listdir(AMFCOLLROOTPATH):
        if os.path.isdir(AMFCOLLROOTPATH+path):
            COLLS.append(path)
        shuffle(COLLS)
        for excludedColl in EXCLUDEDCOLLS:
            if coll not in EXCLUDEDCOLLS:
                print time.time(),'auverting',coll
...
while(True):
    ...
    if __name__ == '__main__':
        ...
        try:
            ...
            print 'System-wide auversion began at',time()
