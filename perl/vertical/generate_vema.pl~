...
sub generate_vema {
  ...
  foreach my $author (keys %{$authors->{'w'}}) {
    ...
    # Time limit, like the depth limit, is pointless; it can only lead to inconsistencies in data
...
#      }
      ...
      # There is no possible means by which to determine the degree to which the tree has been explored because we have not fully explored the network.
      ...
      # 11/21/11
      # Setting a maximum number of nodes only ends the vertical integration calculations prematurely and yields inconsistent data
      # The vertical integration calculations explore each branch to a specified depth, and then explore the neighboring branch to the same specified depth
      # The entire tree cannot be explored in any alternative manner, save for either attempting to explore the entire network, or attempting to explore a component of the network (perhaps)
...
###################################################################################################
      ...
      while($probe_dist > 0) {
        ...
        # 11/21/11
        # No time limits here: This would produce inconsistent data!
        # Check the time limit
        # if(($INIT_TIME - time) >= $TIME_LIMIT) {
          # if(not $TIMEOUT) {
            # warn("Time limit of $TIME_LIMIT seconds exceeded by the network exploration - ending the exploration.\n");
...
########################################################
        ...
        while($inner_counter < $n_index) {
          ...
          # 11/21/11 All deprecated
          # This never affected the ability to generate vertical integration data, so it was not modified during the migration to MongoDB
          ...
          # It's also somewhat pointless - again, we haven't fully explored the tree until AFTER the calculations are finished.
