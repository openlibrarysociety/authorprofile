...
require '/home/aupro/ap/perl/bin/vertical/header.pl';
require '/home/aupro/ap/perl/bin/vertical/get_text_total_auth.pl';
require '/home/aupro/ap/perl/bin/vertical/find_aun_for_auth_texts.pl';
require '/home/aupro/ap/perl/bin/vertical/find_aun_for_aun_text.pl';
#require 'proc_args.pl';
#require 'proc_input.pl';
require '/home/aupro/ap/perl/bin/vertical/create_vema_db.pl';
require '/home/aupro/ap/perl/bin/vertical/create_noma_db.pl';
...
#my $VEMA_DB_DIR="$home_dir/ap/var";

if(not -d $VEMA_DB_DIR) {

  eval { `mkdir $VEMA_DB_DIR`; };
  if($@) { die $!; }

  if(not -d $VEMA_DB_DIR) { die "Fatal Error: Could not create $VEMA_DB_DIR!"; }
}

my $g_vema_db_file="$VEMA_DB_DIR/vema.db";
...
#    $auma=mp_retrieve($auma_pack_file);
    ...
    if($VERBOSE > 1) { print "loading auma\n"; }
#    $auma=mp_retrieve($auma_pack_file);
    $auma=AuthorProfile::Common::json_retrieve($auma_file);
  }
...
my $g_noma_db_file="$VEMA_DB_DIR/noma.db";
...
sub parse_file {
  ...
  if($DEBUG) { if($VERBOSE) { print "\"Dry run\" mode enabled: No transactions with the vertical integration database will be performed.\n"; } }
...
#    if ($VERBOSE) { print("Vertical integration data successfully generated for $file.\n"); }
#    return 0;
#  }
...
#  &AuthorProfile::Common::close_db($g_noma_db);
  ...
  if ($VERBOSE > 0) { print("Vertical integration data successfully generated for $file.\n"); }
...
#  my $root_element=&get_root_from_file($aun_file);
  ...
  if ($VERBOSE > 0) { print("Searching for neighbors for $aunex...\n"); }
  ...
  if(defined($aunexes->{'a'}->{$aunex})) {
    ...
    if ($VERBOSE > 0) { print("Found a total of $tot_aun neighbors for $aunex.\n"); }
  }
  return $aunexes;
}
...
sub generate_vema {
  ...
  foreach my $init_auth (keys %{$edges->{'w'}}) {
    ...
    # 11/02/12: The time limit has been disabled.
    # Check the time limit
#    if(($g_init_time - time) >= $g_time_limit) {
#      if(not $g_timeout) {
...
#    }
    ...
    # To give a visualization of the completion of vema exploration.
    if ($VERBOSE > 0) { print("Processing author $init_auth...\n"); }
    ...
    foreach my $init_aun (keys %{$edges->{'w'}->{$init_auth}}) {
      ...
      # To give a visualization of the completion of vema exploration.
      if ($VERBOSE > 0) { print("Found primary aunex $init_aun for $init_auth...\n"); }
        ...
        while($inner_counter < $n_index) {
          ...
          if($null_var) {
            ...
            if(
               (
                (
                 ($current_t_prog - $noma_record->{'t_gen_progress'}) > 0 and ($current_t_prog - $noma_record->{'t_gen_progress'}) > 0.001)
                or 
                ($current_t_prog - $noma_record->{'t_gen_progress'}) < -0.001
               )
               or
               (not exists $noma_record->{'t_gen_progress'})
              ) {
